...
- `/oncall <TICKET-KEY>` - Trigger the full on-call pipeline for a Jira ticket
  _(spins up 4 parallel agents → posts Slack briefing with Approve/Request Changes)_

### Agent & Session
- `/help` - Show this help
- `/exit` or `/quit` - Exit
- `/clear` - Clear conversation history
- `/stats` - Session statistics (turns, token usage)
...
- `/approval <mode>` - Change approval policy (auto / on-request / yolo / never)

### Tools & Integrations
- `/tools` - List all available tools (includes Jira, Slack, GitHub)
- `/mcp` - MCP server status and connected tools
- `/telegram` - Start Telegram bot for remote access

## Approval Modes
| Mode | Behaviour |
|------|-----------|
| `auto` | Auto-approve reads, confirm writes |
| `on-request` | Confirm all tool calls |
| `yolo` | No confirmation (use carefully) |
